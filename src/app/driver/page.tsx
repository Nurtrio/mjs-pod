...
export default function DriverSelectPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-56px)] flex-col items-center justify-center bg-background px-6 py-10">
      {/* Logo area */}
      ...
      <p className="mb-10 text-[17px] text-muted">Select your name to start</p>
        ...
        {drivers.map((driver) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-5 rounded-2xl bg-card p-5 text-left transition-all duration-150 active:scale-[0.97] active:bg-card-hover"
              style={{
            ...
            >
              ...
              <div className="flex-1">
                <span className="text-[22px] font-bold text-foreground">{driver.name}</span>
                {driver.pin && (
                  <p className="mt-0.5 text-[14px] text-muted">PIN protected</p>
                )}
              </div>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="h-5 w-5 text-muted-2">
      ...
      {pinDriver && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 backdrop-blur-xl">
          <div
            className="w-full max-w-sm rounded-[28px] bg-card p-8"
            style={{ animation: 'spring-in 0.35s ease-out', boxShadow: '0 8px 40px rgba(0,0,0,0.15)' }}
          >
            <div className="mb-8 flex flex-col items-center gap-3">
              <div
                className="flex h-[72px] w-[72px] items-center justify-center rounded-full text-[24px] font-bold text-white"
                style={{ backgroundColor: getDriverColor(pinDriver.name) }}
              >
                {pinDriver.name.charAt(0).toUpperCase()}
              </div>
              <h2 className="text-[22px] font-bold text-foreground">{pinDriver.name}</h2>
              <p className="text-[15px] text-muted">Enter your 4-digit PIN</p>
            </div>
            ...
            <div className="mb-8 flex justify-center gap-5">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`h-[14px] w-[14px] rounded-full transition-all duration-200 ${
                    i < pin.length
                      ? pinError
                        ? 'bg-danger scale-110'
                        : 'bg-accent scale-110'
            ...
            {pinError && (
              <p className="mb-4 text-center text-[15px] font-medium text-danger">
                Wrong PIN. Try again.
              </p>
            )}
              ...
              {['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'backspace'].map((key) => {
                ...
                return (
                  <button
                    ...
                    className="flex h-16 w-16 items-center justify-center justify-self-center rounded-full bg-background text-[24px] font-medium text-foreground transition-all duration-100 active:bg-[#d1d1d6] active:scale-[0.92]"
            ...
            <button
              ...
              className="mt-8 w-full py-2 text-center text-[17px] font-medium text-ios-blue transition-opacity active:opacity-60"
              style={{ WebkitTapHighlightColor: 'transparent' }}
            >
              Cancel
            </button>
